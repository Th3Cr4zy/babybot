...
baby.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ *BEM VINDO(A) AO GRUPO! ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Apresentacao/Membro(só se quiser mamaco)!!! \n➸ Nome : \n➸ Idade : \n➸ Recebe Bolsa Familia: \n➸ Vai Seguir As Regras Deste Grupo?!!! \n➸ Instagram? \nNÃO INVADA O PV DOS MEMBROS! \n *___________________________*\nMinorias serao banidas🤡`
				...
				teks = `ALA, SAIU DO GRUPO... @${num.split('@')[0]}👋* \n_Adeus, espero que não volte😂😂`
				let buff = await getBuffer(ppimg)
				baby.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return baby.sendMessage(from,`Seu limite subiu\n\n_Nota : limite pode ser obtido por meio de ${prefix}buylimit dan naik level_`, text,{ quoted: mek})
...
*/
				...
				if (isBanned) return reply(nad.baned())
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie fotos / videos / gifs com legendas \n${prefix}sticker (duração do sticker video 1-9 segundos)`)
					}
					break
				case 'nulis':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Cadê o texto? Exemplo : ${prefix}nulis Crazy é foda`)
				nul = body.slice(7)
				reply('「❗」ESPERE, ESTOU ESCREVENDO O TEXTO')
				...
				if (args.length < 1) return reply(`Cade o texto, man? Exemplo : ${prefix}nulis1 Crazy bondoso`)
				ramlan = body.slice(8)
				reply('「❗」ESPERE ESTOU ESCREVENDO O TEXTO')
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
				if (args.length < 1) return reply(`Cade o texto man? Exemplo : ${prefix}nulis2 Crazy fofo`)
				laysha = body.slice(8)
				reply('「❗」WAIT ESTOU ESCREVENDO O TEXTO')
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}ninjalogo Crazy & Gans`)
				reply(nad.wait())
		...
		case 'halloweentext':
				...
				ween = body.slice(15)
				if (ween.length > 10) return reply('O texto é mt longo baka, só até 9 caracteres')
				reply(nad.wait())
		    ...
		    break
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}pornhub Crazy & Hub`)
				reply(nad.wait())
		...
		case 'textlight':
		    ...
		    break
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}glitchtext Crazy & Gans`)
					reply(nad.wait())
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, 'Coloca o código do idioma tipo ${prefix}tts id Olá Crazy', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return baby.sendMessage(from, `Teksnya mana kak | Exemplo : ${prefix}tts id ah yamate kudasai`, text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('O texto é muito longo fdp')
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (!isQuotedSticker) return reply('Responda ou marque o adesivo que você deseja transformar numa foto, baka! >_<')
					reply(nad.wait())
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(nad.stikga())
						buffer = fs.readFileSync(ran)
						baby.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\nExemplo : ${prefix}bikinquote aku bukan boneka & Kata Crazy`
					if (args.length < 1) return reply(pref)
					reply(nad.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					baby.sendMessage(from, buffer, image, {caption: 'Aqui >_<', quoted: mek})
					await limitAdd(sender)
					break
                case 'meme':
                if (isBanned) return reply(nad.baned())
                ...
                if (namaUser.length >= 30) return reply(`Seu nome significa cú :)`)
...
// PREMIUM
		 ...
		 case 'tomp3':
				...
				if (!isPrem) return reply(nad.premium())
                	baby.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('*Manda o vídeo, baka-_-*')
					reply(nad.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await baby.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Yahh deu erro:(')
				...
				if (!isPrem) return reply(nad.premium())
					...
					break
				case 'wiki':
					if (args.length < 1) return reply('digite as palavras-chave')
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
				    if (isLimit(sender)) return reply(nad.limitend(pusname))
				...
				case 'yutubdl':
					...
					break
				case 'tiktod':
					if (args.length < 1) return reply('Cadê o url, hum? Baka')
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('ESSA URL NÃO É VALIDA U.U')
					anu = await fetchJson(`https://api.zeks.xyz/api/tiktok?url=${args[0]}&apikey=apivinz`,)
					reply('[AGUARDE] Processo em andamento')
...
*/
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					setTimeout( () => {
					baby.sendMessage(from, '_Mais 10 segundos..._', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					baby.sendMessage(from, '_Mais 20 segundos..._', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					baby.sendMessage(from, '_Mais 30 segundos..._', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					baby.sendMessage(from, ngebuff, image, { caption: '_Adivinha mano!! não posso responder a doações :v_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break
				case 'bisakah':
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					const bisa =['Claro que eu posso!','Não posso!','Hmmm não sei','Repete, não entendi']
					const keh = bisa[Math.floor(Math.random() * bisa.length)]
					baby.sendMessage(from, 'Pertanyaan  : *'+bisakah+'*\n\nJawaban : '+ keh, text, { citado: mek })
					await limitAdd(sender)
					break
				case 'kapankah':
				case 'quando':
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					const kapan =['Nunca','Depois de Amanhã','Agora mesmo','Daqui 4 dias','Daqui 5 dias',' Daqui 6 dias',' Daqui 1 semana','Daqui 2 semanas','Daqui 3 semanas','Daqui a 1 mês','Daqui 2 meses','Daqui 3 meses','Daqui 4 meses','Daqui 5 meses','Daqui 6 meses','Daqui a 1 ano','Daqui a 2 anos','Daqui a 3 anos','Daqui a 4 anos','Daqui a 5 anos','Daqui a 6 anos','Daqui a 1 século','Tomanocu']
           ...
           if (isLimit(sender)) return reply(nad.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Sim','Não','Pode ser','Repete, não entendi']
...
case 'neontext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}neontext Crazy`)
					neon = body.slice(10)
					reply('「❗」WAIT GANS')
					...
					break					
           case 'seberapagay':
           case 'medirviadagemdo':
           if (isBanned) return reply(nad.baned())
           if (!isRegistered) return reply(nad.noregis())
           if (isLimit(sender)) return reply(nad.limitend(pusname))
					gay = body.slice(13)
		   anu = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`, {method: 'get'})
		   hasil = `Veja o quão gay é ${gay}\n\n\nPorcentagem Gay : ${anu.persen}%\nAlerta!!! : ${anu.desc}`
		   reply(hasil)
		   await limitAdd(sender)
					break	
                case 'truth':
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「 SUCESSO 」OS RECURSOS DE BEM-VINDO NO GRUPO FORAM ATIVOS*')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/group/welkom.json', JSON.stringify(welkom))
						reply('*「  SUCESSO 」RECURSO DE BEM-VINDO DESATIVADO NO GRUPO*')
					} else {
						reply(nad.satukos())
					}
					break
				...
				if (!isRegistered) return reply(nad.noregis())
					if (isLimit(sender)) return reply(nad.limitend(pusname))
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}blackpink Crazy`)
					pink = body.slice(11)
                 ...
                 case 'event':
                  if (isBanned) return reply(nad.baned())                 
					...
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*RECURSOS DE EVENTO FORAM ATIVOS*')
						event.push(from)
				...
				case 'simi':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「 SUCESSO 」ATIVADO OS RECURSOS DO SIMI NO GRUPO*')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/group/simi.json', JSON.stringify(samih))
				...
				case 'nsfw':
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*「 SUCESSO 」ATIVADO OS RECURSOS NSFW NO GRUPO*')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(nsfw))
                  ...
                  if (isBanned) return reply(nad.baned())				
					...
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('TA DESATIVADO? BAKA')
						antilink.push(from)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('「 SUCESSO」ATIVADO ANTI LINKS NO GRUPO')
						baby.sendMessage(from, `ALERTA!!! Se nao for administrador nao envie links de grupo NEM O DAQUI SENAO VAI SER EXPURGADO`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('SO ESPERAR')
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))			
					if (!isGroup) return reply(nad.groupo())
					teks = `*LISTA DOS ADM DO GRUPO* _${groupMetadata.subject}_\n*TOTAL* : ${groupAdmins.length}\n\n`
					no = 0
				...
				if (!isRegistered) return reply(nad.noregis())	
					...
					if (args[0].startsWith('08')) return reply('Use o código do país bro')
					try {
				...
				if (!isRegistered) return reply(nad.noregis())	     	
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marcou alguem? marque o desgraçado')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Pedido aceito, pé no saco neutralizado :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						baby.groupRemove(from, mentioned)
					} else {
						mentions(`Pedido aceito, pé no saco neutralizado @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						baby.groupRemove(from, mentioned)
					}
					break
                case 'hidetag':
           ...
           case 'setname':
                ...
                baby.sendMessage(from, '「 SUCESSO 」Nome do grupo alterado', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isRegistered) return reply(nad.noregis())                
                if (!isGroup) return reply(nad.groupo())
                ...
                baby.sendMessage(from, '*「 SUCESSO 」Desc do grupo alterada', text, {quoted: mek})
					break
           case 'demote':
                if (!isRegistered) return reply(nad.noregis())           
					if (!isGroup) return reply(nad.groupo())
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você quer rebaixar a membro comum!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
                ...
                if (!isRegistered) return reply(nad.noregis())				
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você quer que vide admin!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Yeee parabéns🥳 Você não é mais um membro comum :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
                ...
                if (!isRegistered) return reply(nad.noregis())				
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque em quem você quer dar um hs!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = '*Tiro na sua cabeça* >_< :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                      ...
                      if (!isRegistered) return reply(nad.noregis())
                      if (isBanned) return reply(nad.baned())   
                      baby.sendMessage(from, 'Adeus galera, foi bom estar com vocês :(', text)
                      }, 0)
                      break																									
/*
]=====> DOWNLOAD MENU <=====[
...
*/
                ...
                addKoinUser('558282132376@s.whatsapp.net', fee)
...
*/
                ...
                if (isLimit(sender)) return reply(nad.limitend(pusname))
						...
						baby.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está o sua neko >_<'})
					await limitAdd(sender)
					break
					case 'nsfwneko':
					if (isBanned) return reply(nad.baned())
                ...
                if (isLimit(sender)) return reply(nad.limitend(pusname))
						...
						baby.sendMessage(from, nikko, image, {quoted: mek, caption: 'Aqui uma neko safadinha e.e'})
					await limitAdd(sender)
					break					
                case 'nekonime':
                if (isBanned) return reply(nad.baned())
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					baby.sendMessage(from, vvibu, image, {quoted: mek, caption: 'Nya neko :v'})
					await limitAdd(sender)
				break
                case 'kpop':
                  if (isBanned) return reply(nad.baned())
                ...
                sendImage(hasil, mek, '*DARK BOSS :V*')
                await limitAdd(sender)
				break										
/*
]=====> OTHER MENU <=====[
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Crazy`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`Porque Crazy ambos bot entregam ${one}Xp >_<`)
                      }else{
...
if (!isRegistered) return reply(nad.noregis())
if (isLimit(sender)) return reply(nad.limitend(pusname))
if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}hartatahta Crazy`)
har = body.slice(12)
...
case 'cloudtext':
				...
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}cloudtext Crazy`)
cloud = body.slice(11)
...
*/
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「TRANSMISSAO CRAZY」*\n\n${body.slice(4)}`)
						}
				...
				case 'bcgc':
				     if (!isOwner) return reply(nad.ownerb())
					if (args.length < 1) return reply('Cadê o texto meu dono>_<')
				...
				case 'clearall':
				 ...
				 baby.chatRead (from)
					...
					reply('Obrigado pelo novo perfil, senpai😗')
					break
				case 'clone':
					if (!isGroup) return reply(nad.groupo())
